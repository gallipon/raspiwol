...
- **ゾンビ耐性**：スリープ復帰後に TCP が half-open のまま「接続済みのつもり」で受信が死ぬゾンビ化を自己修復する。①自己エコー方式のハートビートで受信経路の死を検出 → 再接続、それでも回復しなければ `os._exit(1)` でクリーン終了し、②外部ウォッチドッグ（タスクスケジューラ 5分間隔）がプロセスを再起動する。③dashboard はエージェントの `agent` リソースを REST read して「🟢 alive / ⚫ down」を表示。④エージェント down 時のみ「**GPIO で寝かせる（最終手段）**」ボタンが現れる——GPIO は成功率 ~60% で起き返すリスクもあるため、**自動発火せず手動明示のみ**にしている。
